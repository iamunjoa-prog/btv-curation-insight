...
def load_data(race_file, gen_file):
    ...
    def read_one(f):
        ...
        raw = pd.read_excel(f, sheet_name=0, header=None, skiprows=3)
        df  = raw.iloc[:, :5].copy()
...
def render_pivot(race_w, gen_w, all_weeks):
    rm = {r['week']: r for _, r in race_w.iterrows()}
...
def export_excel(race_w, gen_w, race_r, gen_r, all_weeks):
    ...
    for wi, week in enumerate(all_weeks):
        ...
        period = race_w[race_w['week']== week]['period'].values
...
with col1:
    st.markdown("**① RACE(개인화) 블록 데이터** <span style='color:#DC2626;font-size:12px'>필수</span>", unsafe_allow_html=True)
    st.caption("3행 헤더 | A: 일자 | B: 블록명 | C: UV | D: PV | E: 매출")
...
if race_file is None or gen_file is None:
    st.markdown("""
    <div style="background:#f0f4ff;border:2px dashed #4f6ef7;border-radius:12px;
        ...
        <div style="font-size:16px;font-weight:700;color:#4f6ef7;margin:10px 0 4px">
            RACE 파일과 일반 블록 파일을 모두 업로드해 주세요</div>
        <div style="font-size:13px;color:#888">
            3행 헤더 기준 &nbsp;·&nbsp; 4행부터 데이터</div>
        <div style="font-size:11px;color:#bbb;margin-top:6px">
            컬럼 순서: 일자(YYYYMMDD) · 블록명 · UV · PV · 매출</div>
...
try:
    ...
    st.error(f"파일 읽기 오류: {e}\n\n💡 엑셀 파일의 보안 해제 여부와 컬럼 순서(A:일자, B:블록명, C:UV, D:PV, E:매출)를 확인해주세요.")
    st.stop()
...
with st.sidebar:
    ...
    st.markdown(f"- 기간: **{race_df['일자'].min().strftime('%Y.%m.%d')}"
                f" ~ {race_df['일자'].max().strftime('%Y.%m.%d')}**")
...
period_info = race_weekly[race_weekly['week']==selected_week]['period'].values
